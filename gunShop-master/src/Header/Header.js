...
const userIcon = (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" width="40" viewBox="0 0 24 24" strokeWidth="1" stroke="white" className="w-6 h-6">
...
const Header = () => {
    const {HeaderHideBoolean,Products} = useSelector(state=>state.Data) 
    const [inputText,setInputText] = useState('');
    const [SearchingElements,setSearchingElements] = useState([])
    useEffect(()=>{HeaderHideBoolean:true},[])
...
<div style={HeaderHideBoolean?{display:'none'}:{display:'grid'}} className="Header ">
    { /* логотип */}
    <div className="div1"> 
    ...
    <div className="div3"> 
     <div className='userAndACartElement'>
        <div className='userAndCartIconsBox'>
            <Link to='cart'><div>{cartIcon}</div></Link>
