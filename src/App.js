...
const App = () => {
  ...
  useEffect(()=>{dispatch(PutData(Data))},[dispatch])
  ...
  useEffect(()=>{
    [...Products]?.map((item) => {
      ...
      switch (text.slice(0,3)) {
        ...
        case 'rif':setriflefed(current=>[...current,item])
        break;

        case 'smo':setSmoothbore(current=>[...current,item])
        break;

        case 'pis':setPistol(current=>[...current,item])
        break;

        case 'opt':setOptic(current=>[...current,item])
        break;

        case 'amm':setAmmunition(current=>[...current,item])
        break;
  ...
  return (
    <div className="App">  
      ...
      <Routes>    
        ...
        <Route path="smoothbore" element={<Smoothbore  products={smoothbore} />} />
